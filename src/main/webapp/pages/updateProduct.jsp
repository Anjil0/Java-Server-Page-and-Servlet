					...
					<form action="${pageContext.request.contextPath}/UpdateProductServlet"
						...
						<div class="form-group">
							<label for="stock-quantity">Stock Quantity</label> <input
								type="number" class="form-control" name="stockQuantity"
						...
						<div class="form-group">
							<label for="color">Color</label> <select class="form-control"
								...
								<option value="white"
									${product.color == 'white' ? 'selected' : ''}>White</option>
								<option value="red" ${product.color == 'red' ? 'selected' : ''}>Red</option>
								<option value="yellow"
									${product.color == 'yellow' ? 'selected' : ''}>Yellow</option>
								<option value="blue"
									${product.color == 'blue' ? 'selected' : ''}>Blue</option>
							</select>
						</div>
						...
						<div class="form-group">
							<label for="connectivity">Connectivity</label> <select
								...
								<option value="Wired"
									${product.connectivity == 'Wired' ? 'selected' : ''}>Wired</option>
								<option value="Wireless"
									${product.connectivity == 'Wireless' ? 'selected' : ''}>Wireless</option>
							</select>
						</div>
						...
						<div class="form-group">
							<label for="noise-cancellation">Noise Cancellation</label> <select
								...
								<option value="Yes"
									${product.noiseCancellation == 'Yes' ? 'selected' : ''}>Yes</option>
								<option value="No"
									${product.noiseCancellation == 'No' ? 'selected' : ''}>No</option>
							</select>
						</div>
						...
						<div class="form-group">
							<label for="driver">Driver</label> <select class="form-control"
								...
								<option value="20MM"
									${product.driver == '20MM' ? 'selected' : ''}>20MM</option>
								<option value="40MM"
									${product.driver == '40MM' ? 'selected' : ''}>40MM</option>
								<option value="50MM"
									${product.driver == '50MM' ? 'selected' : ''}>50MM</option>
							</select>
						</div>
						<div class="form-group">
						...
						<div class="btn-container d-flex gap-3">
							<button type="submit" class="btn darkbg">Update</button>
							<!-- cancel Btn -->
							<a href="${pageContext.request.contextPath}/AdminProductServlet" class="btn darkbg">Cancel</a>
						</div>
					</form>
				</c:forEach>
			</div>
