...
<body>
	...
	<section id="orders"
		style="text-align: center; padding: 10px; margin-top: 10px; background-color: #ebe7e7;">
		<h2 style="text-align: center; margin: 10px;">Orders</h2>
		<table class="orderTable text-white">
			<thead>
				<tr>
					<th>Order ID</th>
					<th>Customer</th>
			...
			<tbody>
				<c:forEach var="order" items="${Allorders}">
					<tr>
						...
						<td>
							<button class="btn darkbg"
								onclick="window.location.href='${pageContext.request.contextPath}/UpdateOrderStatusServlet?orderId=${order.orderId}&orderStatus=' + getUpdatedOrderStatus(this)">Update</button>
						</td>
					</tr>
				</c:forEach>
			</tbody>
	...
	<script>
		document.addEventListener('DOMContentLoaded', function() {
			// Call handleStatusChange for each select element after the page content is loaded
			var statusOrders = document.querySelectorAll('.statusOrder');
			statusOrders.forEach(function(selectElement) {
		...
		function getUpdatedOrderStatus(button) {
			// Get the selected status value relative to the clicked button
			var status = button.closest('tr').querySelector('.statusOrder').value;
			return status;
		}
	</script>
</body>
